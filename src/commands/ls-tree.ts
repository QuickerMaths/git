...
export async function lsTree(gitRoot: string, argvHash: string, argvRecursive: boolean, argvShowTree: boolean) {
    const tree = new Tree();
    const treeRoot = new TreeObject(FileMode.DIR, '', '', argvHash);
    tree.treeRoot = treeRoot;

    const treeArray = [treeRoot];
    await processTree(gitRoot, tree, treeArray, argvRecursive);
